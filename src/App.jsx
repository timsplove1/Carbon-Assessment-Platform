    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      <nav className="bg-white border-b px-8 py-4 flex justify-between items-center shadow-md sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-black leading-none uppercase tracking-tighter">組織碳盤查系統</h1>
            <p className="text-[10px] text-slate-400 font-bold uppercase">Activity Data Portal</p>
          </div>
        </div>
        <div className="flex gap-6 items-center">
          ...
          {user.role === 'admin' && (
            <button onClick={() => setIsAdminView(!isAdminView)} className="px-4 py-2 bg-indigo-50 text-indigo-700 rounded-lg font-bold text-sm hover:bg-indigo-100 transition-all border border-indigo-100">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-indigo-600 p-6 font-sans">
      <div className="bg-white p-10 rounded-3xl shadow-2xl w-full max-w-sm">
        <h2 className="text-3xl font-black text-center mb-2 text-slate-800 tracking-tighter">登入系統</h2>
        <p className="text-slate-400 text-center text-sm mb-10 font-medium tracking-tight">請輸入由管理員核發之帳密</p>
        <form onSubmit={handleLogin} className="space-y-6">
          <input name="username" placeholder="帳號" required className="w-full bg-slate-50 border-none rounded-xl p-4 focus:ring-2 focus:ring-indigo-500 outline-none transition-all shadow-inner" />
          <input name="password" type="password" placeholder="密碼" required className="w-full bg-slate-50 border-none rounded-xl p-4 focus:ring-2 focus:ring-indigo-500 outline-none transition-all shadow-inner" />
...
function UserForm({ user }) {
  ...
  return (
    <div className="bg-white p-8 lg:p-12 rounded-3xl shadow-xl border border-slate-100 animate-in fade-in slide-in-from-bottom-2">
      <div className="mb-12">
        <h2 className="text-3xl font-black text-slate-800 tracking-tight">數據採集表單</h2>
        <p className="text-slate-400 font-medium mt-2 uppercase text-xs tracking-widest">填報部門：<span className="text-indigo-600 font-bold border-b-2 border-indigo-100">{user.dept_name}</span></p>
      </div>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <button type="button" onClick={() => append({ item_name: "", value: "", url: "" })} className="w-full border-2 border-dashed border-slate-200 py-4 rounded-2xl text-slate-400 font-bold hover:bg-white hover:border-indigo-300 hover:text-indigo-600 transition-all">+ 新增一筆活動紀錄項目</button>
      ...
      {selectedDept && (
        <div className="space-y-6">
          <form onSubmit={addItem} className="flex gap-4 p-6 bg-emerald-50 rounded-2xl border border-emerald-100">
            <input name="item_name" placeholder="輸入新增排放源項目名稱 (如: 堆高機用油)" required className="flex-1 border-none bg-white rounded-xl p-4 shadow-sm outline-none" />
  ...
  return (
    <div className="space-y-10">
      <form onSubmit={addDept} className="flex gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
        <input name="name" placeholder="輸入新部門名稱" required className="flex-1 border-none bg-white rounded-xl p-4 shadow-sm outline-none focus:ring-2 focus:ring-indigo-500" />
...
function UserManager({ depts }) {
  ...
  return (
    <div className="max-w-2xl bg-slate-50 p-8 rounded-3xl border border-slate-200 shadow-inner">
      <h3 className="text-xl font-black text-slate-800 mb-8 underline decoration-indigo-200 underline-offset-8">建立新的填報同仁帳號</h3>
