    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      <nav className="bg-white border-b px-8 py-4 flex justify-between items-center shadow-md sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-black tracking-tight">組織碳盤查採集系統</h1>
        </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-indigo-600 p-6">
      <div className="bg-white p-10 rounded-3xl shadow-2xl w-full max-w-sm animate-in zoom-in-95 duration-300">
        <h2 className="text-3xl font-black text-center mb-8 text-slate-800 tracking-tighter">系統登入</h2>
...
function UserForm({ user }) {
  ...
  return (
    <div className="bg-white p-8 lg:p-12 rounded-3xl shadow-xl border border-slate-100 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div className="mb-12">
        <h2 className="text-3xl font-black text-slate-800 tracking-tight">問卷填報：{user.dept_name}</h2>
        <p className="text-slate-400 font-medium mt-2">請選取項目並貼上 Google Drive 佐證連結</p>
      </div>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          return (
            <div key={field.id} className="relative p-8 bg-slate-50 rounded-2xl border border-slate-200 grid grid-cols-1 md:grid-cols-12 gap-6 items-end group shadow-sm transition-all hover:shadow-md">
              <div className="md:col-span-3">
                ...
                <select {...register(`records.${index}.item_name`)} required className="w-full border-none bg-white rounded-xl p-3 shadow-sm focus:ring-2 focus:ring-indigo-500 outline-none">
              ...
              {selected === 'other' && (
                <div className="md:col-span-3 animate-in fade-in zoom-in-95">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">項目名稱</label>
              ...
              <div className="md:col-span-3">
                ...
                <input {...register(`records.${index}.url`)} required placeholder="https://drive..." className="w-full rounded-xl p-3 shadow-sm text-indigo-600 font-bold" />
              </div>
        ...
        <button type="button" onClick={() => append({ item_name: "", value: "", url: "" })} className="w-full border-2 border-dashed border-slate-200 py-4 rounded-2xl text-slate-400 font-bold hover:bg-white hover:border-indigo-300 hover:text-indigo-600 transition-all">+ 新增紀錄項目</button>
  ...
  return (
    <div className="bg-white p-8 lg:p-12 rounded-3xl shadow-xl border border-slate-100 min-h-[600px]">
      <div className="flex gap-8 border-b border-slate-100 mb-10 overflow-x-auto">
        <button onClick={() => setActiveTab("catalog")} className={`pb-4 font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'catalog'? "text-indigo-600 border-b-2 border-indigo-600" : "text-slate-300"}`}>清單維護</button>
  ...
  return (
    <div className="space-y-8">
      <div className="p-6 bg-indigo-50 rounded-2xl border border-indigo-100">
        <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest block mb-2">選取維護部門</label>
    ...
    <div className="max-w-2xl bg-slate-50 p-8 rounded-3xl border border-slate-200 shadow-inner">
      ...
      <form onSubmit={addUser} className="space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <input name="username" placeholder="設定帳號" required className="w-full border-none bg-white rounded-xl p-4 shadow-sm focus:ring-2 focus:ring-indigo-500 font-medium" />
          <input name="password" type="password" placeholder="設定密碼" required className="w-full border-none bg-white rounded-xl p-4 shadow-sm focus:ring-2 focus:ring-indigo-500 font-medium" />
        </div>
