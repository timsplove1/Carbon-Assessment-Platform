  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {depts.map(d => (
          <div key={d.id} className="p-6 bg-white border border-slate-200 rounded-2xl flex justify-between items-center group shadow-sm">
...
function CatalogManager({ depts }) {
  ...
  const addItem = async (e) => {
    ...
    alert("已成功加入清單項目"); e.target.reset();
      ...
      {selectedDept && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {items.map(i => (
              <div key={i.id} className="p-5 bg-white border border-slate-200 rounded-2xl flex justify-between items-center group hover:border-indigo-300 transition-all">
    ...
    <div className="max-w-2xl bg-slate-50 p-10 rounded-3xl border border-slate-200 shadow-inner">
      ...
      <form onSubmit={addUser} className="space-y-6">
        <div className="grid grid-cols-2 gap-4">
          ...
          <input name="password" type="password" placeholder="設定登入密碼" required className="rounded-xl p-4 shadow-sm border-none outline-none focus:ring-2 focus:ring-indigo-500 font-medium" />
        </div>
        <select name="dept_id" required className="w-full rounded-xl p-4 shadow-sm border-none outline-none focus:ring-2 focus:ring-indigo-500 font-bold text-slate-700">
